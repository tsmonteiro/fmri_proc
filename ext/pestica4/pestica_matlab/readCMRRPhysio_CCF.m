function [fext, fcard, fresp, physio] = readCMRRPhysio_CCF(fn,TR,samplerate,downsamplerate)
% -------------------------------------------------------------------------
% readCMRRPhysio.m
% -------------------------------------------------------------------------
...
%
% modified by Wanyong Shin, CCF, 2016

if ~exist('TR');              TR=0.8;             end;  % sec
if ~exist('samplerate');      samplerate=400;     end;  % 1/sec
if ~exist('downsamplerate');  downsamplerate=50;  end;  % 1/sec

sampleNinTR = samplerate*TR;  %320
downsamplingfactor = samplerate/downsamplerate;

% this is the file format this function expects; must match log file version
ExpectedVersion = 'EJA_1';

% say hello
fprintf('\nreadCMRRPhysio_CCF: intialized by E. Auerbach, CMRR, 2015\n\n');
fprintf('                  : modified by Wanyong Shin, CCF, 2016\n\n');
...
% read in the data
[SliceMap, timestamp, phyval, UUID1, NumSlices, NumVolumes, FirstTime, LastTime] = readParseFile(fnINFO, 'ACQUISITION_INFO', ExpectedVersion, 0, 0);
...
[ECG, timestamp, phyval, UUID2] = readParseFile(fnECG, 'ECG', ExpectedVersion, FirstTime, ExpectedSamples);
if (~strcmp(UUID1, UUID2)), error('UUID mismatch between Info and ECG files!'); end

[RESP, timestamp_resp, val_resp, UUID3] = readParseFile(fnRESP, 'RESP', ExpectedVersion, FirstTime, ExpectedSamples);
if (~strcmp(UUID1, UUID3)), error('UUID mismatch between Info and RESP files!'); end

[PULS, timestamp_card, val_card, UUID4] = readParseFile(fnPULS, 'PULS', ExpectedVersion, FirstTime, ExpectedSamples);
if (~strcmp(UUID1, UUID4)), error('UUID mismatch between Info and PULS files!'); end

[EXT, timestamp, phyval, UUID5] = readParseFile(fnEXT, 'EXT', ExpectedVersion, FirstTime, ExpectedSamples);
...
ext_trig = find(diff(physio.EXT) == 1)+1;
tp_ds = ext_trig(1):downsamplingfactor:ext_trig(end)+sampleNinTR-1;

fext = zeros(size(tp_ds));
exton=(find(physio.EXT));

for n = 1:length(tp_ds)
  if find(exton==tp_ds(n))
    fext(n)=1;
  end
end

tmp1 = find(diff(fext)==-1);
tmp2 = find(diff(fext)==0)+1;
for n = 1:length(tmp1)
  if find(tmp2==tmp1(n))
    fext(tmp1(n))=0;
  end
...
fresp=pchip(double(timestamp_resp),double(val_resp),double(tp_ds));
fcard=pchip(double(timestamp_card),double(val_card),double(tp_ds));

% normalization
fcard =(fcard-mean(fcard))/std(fcard);
fresp =(fresp-mean(fresp))/std(fresp);


%--------------------------------------------------------------------------

function [arr, timestamp, phyval, varargout] = readParseFile(fn, LogDataType, ExpectedVersion, FirstTime, ExpectedSamples)
    ...
    elseif (~isempty(line))
        ...
        if (~isstrprop(datacells{1}{1}(1), 'digit'))
            ...
            if (strcmp(LogDataType, 'ACQUISITION_INFO'))
                ...
                timestamp=[timestamp; curstart];
                phyval = [phyval;curvalue]; 
