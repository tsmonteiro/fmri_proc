function retroicor_pestica(ep2d_filename,card,resp,M,mask_filename)
...
for z=1:zdim
  ...
  A = [card_vols(zmb,:)' resp_vols(zmb,:)' ones(tdim,1)];
  for y=1:ydim
    for x=1:xdim
      if mask(x,y,z)
...
OptOut.Scale = 0;
OptOut.OverWrite= 'y';
OptOut.Prefix = strcat(ep2d_filename(1:strfind(ep2d_filename,'+')-1),'.retroicor_pestica');
OptOut.verbose = 0;
